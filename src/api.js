...
class API {
    ...
    async finishTestItem (projectName, testItemId, options) {
        process.stdout.write(`[${filename}] finishTestItem: /item/${testItemId}\n`);
        try {
    ...
    async sendLog (projectName, options) {
        ...
        try {
            ...
            if (options.file) {
                ...
                process.stdout.write(`[${filename}]send file:${options.file.name}\n`);
                //request body 
                ...
                process.stdout.write(`[${filename}]sendLog ${options.message}\n`);
                this.handleResponse(await this.client.post(`/${projectName}/log`, options));
            }
        }
        catch (error) {
            process.stdout.write(`[${filename}]sendLog error${error.message}\n`);
            this.handleError(error);
        }
        process.stdout.write('[' + filename + "]exit sendLog\n");
    }
    ...
    handleResponse (response) {
        if(response.request.path != undefined)
            process.stdout.write(`[${filename}] handleResponse path: ${response.request.path}\n`); 
        process.stdout.write(`[${filename}] handleResponse id: ${response.data.id}\n`);
     ...
     */
    handleError (error) {
        process.stdout.write(`[${filename}]!!! handleError: ${error.errorMessage}\n`);
